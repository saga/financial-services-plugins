...
print(f"Found {len(translation_list)} files to translate.")
...
for i, (src_rel, dst_rel) in enumerate(translation_list):
    ...
    if os.path.exists(dst_full) and os.path.getsize(dst_full) > 10:
        print(f"[{i+1}/{len(translation_list)}] Skipped (already exists): {dst_rel}")
        skip_count += 1
        continue

    print(f"[{i+1}/{len(translation_list)}] Translating {src_rel} -> {dst_rel}...")
    ...
    try:
        ...
        print(f"  -> Error: claude command failed (exit {e.returncode})")
