...
这份文件是"Excel 创作（XLSX Author）"技能的详细说明。在 Month-End Closer 的上下文中，XLSX Author 负责将月末结账的应计时间表、滚调表和差异说明组织成一个"结账包"Excel 文件，供财务主管审核签字。

---

### 1. 月末结账 Excel 的"四大 Sheet"

| Sheet | 内容 | 签字人 |
|-------|------|--------|
| Trial Balance | 试算平衡表——所有科目的期初/本期变动/期末余额 | 会计师 → 主管 |
| Accruals | 应计分录清单 + 支持文件索引 | 会计师 → 主管 |
| Roll Forward | 关键科目（固定资产/递延收入）的期初到期末滚动 | 会计师 → 主管 |
| Variance | 损益表科目 vs 前期 vs 预算，波动原因简述 | 会计师 → 主管 → CFO |
...
| 原则 | 说明 |
|------|------|
| 可追溯 | 每个应计分录必须挂一个支持文件（发票/合同/邮件） |
| 一致性 | 同一个科目不允许在不同 Sheet 中出现不同的数字 |
| 签字线 | 每个 Sheet 必须有审核签字行 |

---

### 给小白的一句话

> Month-End Closer 的 XLSX Author 就是月末的"账本装订员"——你做完应计、滚调和差异分析后，不能把零散的数字发给主管审——你要把它们整齐地装订成一个 Excel 包：第一页试算平衡、第二页应计清单、第三页滚调、第四页差异解释。每个数字都必须能追溯到源文件。主管的签字不是"走过场"——签字意味着"我对这里的每一个数字负个人责任"。
