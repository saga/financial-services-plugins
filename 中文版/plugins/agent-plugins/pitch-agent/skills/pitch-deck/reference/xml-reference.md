...
这份文件是"XML 参考（XML Reference）"文档的详细说明。当投行需要批量修改 PPT 或通过代码自动化时，理解 PPT 的底层 XML 结构是关键。
...
**类比：**
一个 .pptx 文件本质上是一个 ZIP 压缩包，里面装着 XML 文件。就像你把一个乐高模型拆开，会发现它由几百个小积木组成——每块积木就是一段 XML 代码。
...
```
my_pitch.pptx（改名为 .zip 解压后）
├── ppt/
│   ├── slides/slide1.xml   ← 第1页所有内容
│   ├── slides/slide2.xml   ← 第2页所有内容
│   ├── slideMasters/        ← 母版
│   └── theme/               ← 配色和字体
├── docProps/                ← 作者、修改时间
└── _rels/                   ← 文件关系
...
| 场景 | XML 的价值 |
|------|-----------|
| 批量修改 50 页的颜色 | 一行正则替换搞定 |
| 修复损坏的 PPT | 直接编辑 XML 抢救 |
| Python 生成 PPT | 底层精确控制 |

---

### 给小白的一句话

> XML Reference 就像投行 PPT 的"内部构造图"——平时你可能永远用不到，但当需要批量修改 100 页中某个元素时，你会发现 PPT 本质上就是一个 ZIP 包里装着 XML 文件。理解了这一点，你就能像黑客一样直接"操作 PPT 的内部代码"，而不仅仅是在界面上点点点。
