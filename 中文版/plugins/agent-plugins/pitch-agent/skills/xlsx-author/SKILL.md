...
这份文件是"Excel 创作（XLSX Author）"技能的详细说明。在投资银行中，Python 生成 Excel 模型是一项核心生产力技能——它让你能自动化地创建带有公式、格式、多 Sheet 的专业级财务模型。

---

### 1. 什么是 Python 生成 Excel？

**类比：**
想象你是一个表格设计师。客户要求"创建一个 100 页的表格，每页格式一样但数字不同"。你不会手动创建 100 次——你会写一个脚本自动生成。

Python 生成 Excel 就是投行界的"表格自动化工厂"——用代码批量生成格式统一的 Excel 文件。

---

### 2. python-openpyxl 的核心能力

| 能力 | 说明 | 例子 |
|------|------|------|
| 创建公式 | 写入 Excel 公式 | `ws['B10'] = '=SUM(B2:B9)'` |
| 设置格式 | 字体、颜色、边框、数字格式 | 千分位逗号、百分比 |
| 多 Sheet | 在一个文件里创建多个 Sheet | IS, BS, CFS |
| 条件格式 | 超过阈值的标红 | 亏损数字标红色 |
| 图表 | 生成 Excel 图表 | 折线图、柱状图 |
...
| 百分比格式 | `0.0%`（一位小数）|
| 倍率格式 | `0.0x` |
| 字体 | Arial 10pt / Calibri 11pt |
| 颜色 | 蓝字 = 输入，黑字 = 公式，绿字 = 链接 |
| 边框 | 顶部边框线 + 底部双线 = 合计行 |
...
| 实践 | 说明 |
|------|------|
| 区分输入/公式 | 用蓝色字体标记手动输入的数字 |
| 保护公式 | 锁定公式单元格，防止用户误删 |
| 命名区域 | 给关键区域命名（如 `RevenueGrowth`） |
| 添加注释 | 在复杂公式旁加注释 |
| 版本管理 | 文件命名包含日期和版本号 |

---

### 5. 真实案例：自动生成可比公司分析 Excel

**背景**：分析师需要生成一份"可比公司分析"Excel，包含 20 家公司的估值倍数

| 手动方式 | Python 方式 |
|---------|------------|
| 手动输入 20 家公司数据 | 从 API 自动获取 |
| 手动计算 20 家 P/E | 公式自动填充 |
| 手动格式化 | 脚本一次性格式化 |
| 总耗时 3 小时 | 总耗时 2 分钟 |
...
> XLSX Author 是投行 Excel 的"批量生产线"——你花 30 分钟写一个 Python 脚本，之后 100 个 Excel 文件都能自动生成，每个都有正确的公式、统一的格式、好看的配色。这在投行场景下非常有用：同一份财务模型，不同客户需要不同的假设数字（"保守假设" vs "乐观假设"），一键重新生成就可以了。但要小心：自动生成的 Excel 需要人工验证——因为如果有 Bug，错误也会被批量复制 100 次。
