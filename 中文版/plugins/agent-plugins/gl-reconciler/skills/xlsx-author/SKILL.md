...
这份文件是"Excel 创作（XLSX Author）"技能的详细说明。在 GL Reconciler 的上下文中，XLSX Author 负责将对账差异和根因分析输出到一个格式规范的 Excel 工作簿中。

---

### 1. GL 对账 Excel 的"标准结构"

| Sheet | 内容 |
|-------|------|
| Summary | 对账日期、资产类别列表、总体结论（通过/有差异需升级） |
| Breaks | 每个差异行：GL 余额、子账余额、差异金额、阈值、是否需升级 |
| Root Cause | 每条差异的追溯结果和一句话根因 |
| Sign-off | 审核人员签字栏 |
...
| 要求 | 说明 |
|------|------|
| 差异按金额排序 | 最大的差异排最前面——管理者先看最严重的 |
| 阈值明确 | 小于阈值的标为"忽略"、大于的标为"需升级" |
| 根因可追溯 | 每条根因必须能追溯到具体的交易 ID |

---

### 给小白的一句话

> GL Reconciler 的 XLSX Author 就是"对账异常报告"——GL 对账员逐笔查出差异并找到根因后，不能只留一堆零散的笔记，必须整理成一个 Excel：第一页总体结论（今天 500 笔交易，7 笔异常，3 笔已解决，4 笔需主管升级），后面逐页列出每个差异的 GL 值、子账值、差异金额和根因。核心：按金额从大到小排序——不要把 $1 的四舍五入放在老板看到的第一行。
