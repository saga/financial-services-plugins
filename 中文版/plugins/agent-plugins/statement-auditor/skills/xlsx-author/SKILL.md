...
这份文件是"Excel 创作（XLSX Author）"技能的详细说明。在 Statement Auditor 的上下文中，XLSX Author 负责将 LP 对账单审计的核对结果和异常清单输出为一个格式规范的 Excel 文件。

---

### 1. LP 对账单审计 Excel 的"标准结构"

| Sheet | 内容 |
|-------|------|
| Reconciliation | 逐 LP 列出每个字段（资本账户余额、分配、费用、NAV） |
| Breaks | 任何不匹配的差异 + 推测原因 |
| Sign-off | 每份对账单"通过/暂扣" + 审计员签名 |
...
| 要求 | 说明 |
|------|------|
| LP 编号对应 | 每个 LP 用唯一 ID 标识——不能把 A 的钱算到 B 头上 |
| 逐字段对比 | 不只是"总余额对比"——每个子字段（管理费、分配、NAV）都要对 |
| 审计痕迹 | 谁在什么时候核对的——有 timestamp |

---

### 给小白的一句话

> Statement Auditor 的 XLSX Author 就是 LP 对账单的"核对底稿"——审计员把每份 LP 对账单和 NAV 数据源逐字段对比后，结果不能只口头报告，必须输出成一个 Excel 让对方看清楚：A 对账单的资本账户余额 $5,234,567 vs NAV 包里的 $5,234,567 ✓，B 对账单的分配金额 $123,456 vs NAV 包里的 $123,450 ✗ 差异 $6——为什么？因为四舍五入？还是计算错误？
