...
这份文件是"首次覆盖报告模板（Initiation Report Template）"的详细说明。在股权研究中，报告模板是"标准化输出"的蓝图——它规定了报告的结构、格式、质量标准，确保每份报告都达到机构级别的专业水准。
...
**类比：**
想象你是一家汽车杂志的编辑，第一次写一篇某款新车型的评测。你需要：
- 外观内饰评价（公司概览）
- 性能测试数据（财务分析）
- 竞品对比（行业分析）
- 综合评分（估值与评级）
...
首次覆盖报告就是金融研究界的"新车评测"——第一次对一家之前从未覆盖的公司进行全面分析。
...
**结构逻辑**：从最重要的信息（投资结论）开始，逐步深入细节（公司、行业、财务），最后补充支撑材料（附录）。

---

### 3. 第 1 页（Investment Update）的"必胜方程"
...
| 当前价 | $42 |
| 上涨空间 | +19% |
| 一句话推荐 | "我们首次覆盖 XYZ，给予买入评级，基于行业顺风+估值合理" |
| 三大投资论点 | 1. 市场增长 2. 竞争优势 3. 催化剂 |
| 财务快照 | 收入、EPS、PE、EV/EBITDA 对比 |
...
| 铁律 | 说明 |
|------|------|
| 高密度呈现 | 每页至少包含文字+图表/表格中的两类 |
| 避免孤岛 | 不要让章节标题独占一页 |
| 图文交错 | 图表嵌入分析叙述中，不堆在文末 |
| 统一视觉 | 字体、配色、图例风格全文一致 |
| 数据优先 | 能用数字就不用空泛描述 |
| 来源必标 | 每张图表标注数据来源 |
| 编号连续 | 所有图表使用连续 Figure 编号 |
| 分页控制 | 不出现大面积空白 |
| Word 交付 | 最终 DOCX，不是 Markdown |
| 超 30 页 | 每份报告 ≥ 30 页 |

---

### 5. 真实案例：一份"教科书级"报告的诞生

**公司**：某 SaaS 公司首次覆盖

| 阶段 | 产出 |
|------|------|
| 第 1 周 | 研究公司基本面，完成 6000 字公司研究 |
| 第 2 周 | 构建三表财务模型，预测 5 年业绩 |
| 第 3 周 | DCF + Comps 估值，得出目标价 |
| 第 4 周 | 生成 28 张图表 |
| 第 5 周 | 汇编、排版、质量控制 |
| 最终 | 32 页正式 DOCX 报告 + XLS 模型 |

**客户反馈**："这是我见过最清晰的首次覆盖——从第 1 页就知道该不该买。"

---

### 给小白的一句话

> 报告模板就是金融研究的"烹饪食谱"——你有了食材（数据）、调味料（洞察）、烹饪工具（Excel/Python），但如果不知道"放多少盐、什么时候加酱油"，做出来的菜可能不堪入口。模板就像一个米其林大厨的标准化配方：第1页放什么（投资结论）、第2-10页放什么（业务分析）、第11-20页放什么（财务和估值），每一页都有明确的"呈现目标"。用模板的好处是：不管谁来写，产出的报告都有"同一个味道"——专业、清晰、可预测。
