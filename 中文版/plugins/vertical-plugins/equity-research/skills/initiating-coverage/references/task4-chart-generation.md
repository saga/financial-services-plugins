...
这份文件是"图表生成（Chart Generation）"任务的详细说明。在首次覆盖报告中，图表是"灵魂"——一份 50 页的研究报告如果没有图表，就像一本没有插图的小说，枯燥难读。图表能一眼传递关键信息，比文字高效得多。
...
| 图表类型 | 英文 | 用途 | 例子 |
|---------|------|------|------|
| 折线图 | Line Chart | 趋势展示 | 股价走势、收入增长 |
| 柱状图 | Bar Chart | 对比 | 各业务线收入对比 |
| 饼图 | Pie Chart | 占比 | 收入构成、成本结构 |
| 散点图 | Scatter Plot | 相关性 | 估值倍数 vs 增长率 |
| 瀑布图 | Waterfall | 变动分解 | EBITDA 从去年到今年的变化 |
| 热力图 | Heatmap | 密度 | 各市场利润率分布 |
| 组合图 | Combo Chart | 多维度 | 收入（柱状）+ 增长率（折线） |

---

### 2. 首次覆盖报告最常用的"10 张经典图表"

| 序号 | 图表 | 位置 | 核心信息 |
|------|------|------|---------|
| 1 | 股价走势图 | 投资总结页 | 当前股价 + 目标价 |
| 2 | 收入构成饼图 | 业务分析 | 各业务贡献占比 |
| 3 | 收入增长折线图 | 财务分析 | 3-5 年收入增长趋势 |
| 4 | 利润率趋势图 | 财务分析 | 毛利率、净利率走势 |
| 5 | 估值倍数对比图 | 估值分析 | vs 同行 P/E, EV/EBITDA |
| 6 | 市场份额柱状图 | 竞争分析 | 公司 vs 竞品份额 |
| 7 | DCF 敏感性热力图 | 估值分析 | WACC vs 增长率矩阵 |
| 8 | 管理层时间线 | 公司介绍 | CEO/CFO 任职时间 |
| 9 | 价格 vs 销量拆解 | 业务驱动 | 业绩增长的驱动力 |
| 10 | 投资回报情景分析 | 投资结论 | 熊/基/牛三种情景 |
...
| 禁忌 | 错误示范 | 正确做法 |
|------|---------|---------|
| 截断 Y 轴 | 从 $90 开始截断，让-$5 看起来像暴跌 | 从 $0 开始，展示真实比例 |
| 三维效果 | 3D 饼图失真好严重 | 用 2D 饼图或条形图 |
| 颜色过多 | 一张图用 10 种颜色 | 统一配色，3-5 种颜色即可 |

---

### 4. 图表来源标注（Source Line）

每一张图表都必须标注数据来源：

```
Source: Company filings, FactSet, Bloomberg, CapIQ, Analyst estimates
```

**为什么重要？** 信息来源标注是金融研究的"学术诚信"——不合规可能被退报告。

---

### 5. 真实案例：一张图毁了一份报告

**背景**：某分析师推荐买入某零售公司，图表显示"同店销售增长 +8%"

**问题**：图表 Y 轴从 +6% 开始，让 +8% 看起来像 "爆炸式增长"

**真相**：前一年增长 +10%，实际上增速在放缓！

**后果**：客户投诉"图表误导"，分析师被内部审查。

---

### 给小白的一句话

> 图表生成就是金融研究的"可视化翻译"——你把 Excel 里密密麻麻的数据，翻译成一张 5 秒钟就能看懂的趋势图。好的图表是一目了然的"一句话结论"（"收入趋势向上"），而不好的图表是"高考数学附加题"（还要用户拿尺子量来算）。最重要的原则是"诚实"——不要通过截断 Y 轴或夸张颜色来误导读者。金融研究卖的是"信任"，一旦一张图被发现造假，整份报告的结论都会被质疑。
